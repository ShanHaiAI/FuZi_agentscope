...
planner_agent, teacher_agent, QA_agent, Exam_agent, ans_gen_agent, marker_agent ,process_agent, strenthen_teacher= [agents[i] for i in range(len(agents))]
# todo：challenger_agent
user_agent = UserAgent()
def main() -> None:
    intro = HostMsg(content="您好，我是夫子，您的智能学习助手。请告诉我您希望学习哪方面的知识。")
    roles = [planner_agent, teacher_agent, QA_agent, Exam_agent, ans_gen_agent, marker_agent,strenthen_teacher, user_agent]
    with msghub(participants=roles, announcement=intro) as tea2stu:
        ...
        strenthen = strenthen_teacher(score)
